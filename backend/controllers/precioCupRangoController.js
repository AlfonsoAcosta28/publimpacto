const { PrecioCupRango } = require('../models');

module.exports = {
  async create(req, res) {
    try {
      const { id_cup, min_cantidad, max_cantidad, precio_unitario } = req.body;

      // Verificar que el nuevo rango no se solape con rangos existentes
      const rangosExistentes = await PrecioCupRango.findAll({
        where: { id_cup },
      ...
      const rango = await PrecioCupRango.create(req.body);
  ...
  async findAll(req, res) {
    try {
      const rangos = await PrecioCupRango.findAll();
      res.json(rangos);
    } catch (err) {
        console.log(err)
  ...
  async findById(req, res) {
    try {
      const rango = await PrecioCupRango.findByPk(req.params.id);
